...
final class SettingsWindowController: NSWindowController, NSTextFieldDelegate {
    ...
    private func makeNotificationsPage() -> NSView {
        ...
        let notificationToggle = NSButton(checkboxWithTitle: "会话结束时发送桌面通知", target: self, action: #selector(sessionNotificationToggled(_:)))
        notificationToggle.state = sessionNotificationPreferences.isEnabled ? .on : .off
        stack.addArrangedSubview(settingRow(title: "桌面通知", detail: "当 Codex/GPT 或 Claude 会话从运行变为空闲时提示。", control: notificationToggle))

        addActionButton(to: stack, title: "配置邮件通知...", action: #selector(openEmailSettings))
        let emailHint = secondaryLabel("邮件配置会保存在 ~/.aistatus/email.json。邮件通知不占用状态栏项目。")
